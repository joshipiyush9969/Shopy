...
export const fetchOrder = () =>{
    return async (dispatch,getState) =>{
      const userId = getState().auth.userId;
        try{
            const response = await fetch(`https://rn-shopyapp-default-rtdb.firebaseio.com/orders/${userId}.json`
            );
...
export const addOrder = (cartItems,totalAmount) => {
    return async (dispatch,getState) =>{
        ...
        const userId = getState().auth.userId;
        const response = await fetch(`https://rn-shopyapp-default-rtdb.firebaseio.com/orders/${userId}.json?auth=${token}`, {
      method: 'POST',
      header:{
